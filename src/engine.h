 ...
 */
#ifndef TENSORRT_ENGINE
#define TENSORRT_ENGINE
#include "NvInfer.h"
#include <chrono>
...
inline void checkCudaError(const char* filename, const char* funcname, int line, cudaError_t code)
{
    if (code != 0)
    {
        ...
        utility::xlogger::getInstance().log_(spdlog::level::critical, filename, funcname, line, errMsg);
        throw std::runtime_error(errMsg);
    }
}

#define checkCudaErrorCode(code)  checkCudaError(__FILENAME__, __FUNCTION__, __LINE__, code)
